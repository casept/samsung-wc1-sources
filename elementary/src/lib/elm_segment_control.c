...
_access_state_cb(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if (it == sd->selected_item)
     ...
     return strdup(E_("State: Unselected"));//[string:error] not included in po file.
}
...
_access_widget_item_register(Elm_Segment_Item *it)
{
   ...
   _elm_access_text_set(ai, ELM_ACCESS_TYPE, E_("Segment Control Item"));//[string:error] not included in po file.
