                    ...
                    Elm_GLView_Mode mode)
{
   ...
   if (mode & ELM_GLVIEW_STENCIL)
     {
        const int mask = 7 << 9;
        if ((mode & mask) == (EVAS_GL_STENCIL_BIT_1 & mask))
          sd->config->stencil_bits = EVAS_GL_STENCIL_BIT_1;
        else if ((mode & mask) == (EVAS_GL_STENCIL_BIT_2 & mask))
          sd->config->stencil_bits = EVAS_GL_STENCIL_BIT_2;
        else if ((mode & mask) == (EVAS_GL_STENCIL_BIT_4 & mask))
          sd->config->stencil_bits = EVAS_GL_STENCIL_BIT_4;
        else if ((mode & mask) == (EVAS_GL_STENCIL_BIT_8 & mask))
          sd->config->stencil_bits = EVAS_GL_STENCIL_BIT_8;
        else if ((mode & mask) == (EVAS_GL_STENCIL_BIT_16 & mask))
