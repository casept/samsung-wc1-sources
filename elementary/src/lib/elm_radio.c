...
_activate(Evas_Object *obj)
{
   ...
   if ((!_elm_config->access_mode) ||
       (_elm_access_2nd_click_timeout(obj)))
     {
        ...
        if (_elm_config->access_mode)
          _elm_access_say(obj, E_("IDS_ACCS_OPT_SELECTED_TTS"), EINA_FALSE);
...
{
   return strdup(E_("IDS_COM_BODY_RADIO_BUTTON_T_TTS"));
}
...
_access_state_cb(void *data __UNUSED__, Evas_Object *obj)
{
   ...
   if (elm_widget_disabled_get(obj)) return strdup(E_("IDS_ACCS_BODY_DISABLED_TTS"));
   if (sd->state) return strdup(E_("IDS_ACCS_OPT_SELECTED_TTS"));

   return strdup(E_("IDS_ACCS_OPT_NOT_SELECTED_TTS"));
}
