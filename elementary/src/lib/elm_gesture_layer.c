             ...
             case ELM_GESTURE_MOMENTUM:
               {
                  ...
                  INF("ELM_GESTURE_MOMENTUM GestureState:%d, [%d,%d]", gesture->state, p->mx, p->my);
               }
               break;
        ...
        Evas_Coord w)
{
   ...
   if ((abs(xx1-xx2) > (w<<1)) || abs(yy1-yy2) > (w<<1))
     return EINA_FALSE;
...
_tap_gesture_check_finish(Gesture_Info *gesture, Evas_Coord tap_finger_size)
{
   ...
   tap_finger_size = tap_finger_size<<1;
...
_tap_gesture_finish(void *data, Evas_Coord tap_finger_size)
{
   ...
   touched_count = evas_event_down_count_get(evas_object_evas_get(gesture->obj));

   if (_tap_gesture_check_finish(gesture, tap_finger_size) && touched_count == 0)
     {
   ...
   switch (pe->event_type)
     {
      ...
      case EVAS_CALLBACK_MOUSE_DOWN:
		 ...
		 touched_count = evas_event_down_count_get(evas_object_evas_get(obj));

         if ((pe->device == 0) && (eina_list_count(pe_list) == 1) && (touched_count == 1))
   ...
   switch (event_type)
     {
      ...
      case EVAS_CALLBACK_MULTI_UP:
        if ((started) && (started == ended))
          {
             st->info.momentum.tx = tm_end;
             st->info.momentum.ty = tm_end;
   ...
   switch (event_type)
     {
      ...
      { /* Here we take care of zoom-start and zoom move */
        ...
        if (!st->zoom_st.timestamp) /* Now scan touched-devices list
          ...
          {
             const Evas* e = evas_object_evas_get(obj);
             if (eina_list_count(sd->touched) < 2 || evas_event_down_count_get(e) < 2)
               return;
             ...
             if (sd->zoom_threshold > st->info.radius)
               {
                  INF("inside zoom threshold=[%d,%d]", sd->zoom_threshold, st->info.radius);
                  return;
               }
                         ...
                         void *data)
{
   ...
   if(idx <= ELM_GESTURE_FIRST || idx >= ELM_GESTURE_LAST)
      return;

   if(cb_type < ELM_GESTURE_STATE_UNDEFINED || cb_type > ELM_GESTURE_STATE_ABORT)
      return;
...
EAPI void
elm_gesture_layer_zoom_threshold_set(Evas_Object *obj, Evas_Coord zoom_threshold)
{
   _elm_gesture_layer_zoom_threshold_set(obj, zoom_threshold);
}

EAPI Evas_Coord
elm_gesture_layer_zoom_threshold_get(const Evas_Object *obj)
{
   return _elm_gesture_layer_zoom_threshold_get(obj);
}
