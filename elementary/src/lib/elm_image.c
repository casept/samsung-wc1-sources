...
static void _elm_image_file_set_do(Evas_Object *obj);

static const char SIG_ASYNC_FILE_SET_ERROR[] = "async,file,set,error"; // tmp
                    ...
                    void *event __UNUSED__)
{
   ...
   // temp code for failure of file_set
   if (evas_object_image_load_error_get(obj) != EVAS_LOAD_ERROR_NONE)
     {
        Evas_Object *self = sd->base.obj;
        ERR("file_set error !!(%p)", self);
        evas_object_smart_callback_call(self, SIG_ASYNC_FILE_SET_ERROR, NULL);
     }
...
_elm_image_async_open_do(void *data, Ecore_Thread *thread EINA_UNUSED)
{
   ...
   int ret;
   size_t size;
   /* [ASYNC] This is a complete hack.
    * Here's what we do: we fopen() and stat() the file so that those will
    * be very fast during evas_object_image_file_set(). We rely on the kernel
    * to cache the file header for us.
    */

   if (sd->async_fd)
     {
        ERR("Duh, there's already an async file open?");
        fclose(sd->async_fd);
        sd->async_fd = NULL;
     }

   sd->async_fd = fopen(sd->async_file, "rb");
   if (!sd->async_fd) return;

   ret = fstat(fileno(sd->async_fd), &st);
   if ( ret < 0 ) return;
   
   size = fread(buffer, 1, sizeof(buffer), sd->async_fd);
   if ( size == 0 ) return;
}
...
_elm_image_async_open_done(void *data, Ecore_Thread *thread EINA_UNUSED)
{
   ...
   // [ASYNC] Based on the fopen/fread hack above we're just gonna start
   // the actual image file set now.

   if (!sd->async_fd)
     ERR("Async file set failed to open the file!");

   // This code comes from _elm_image_smart_internal_file_set (EFL 1.12)
   _elm_image_file_set_do(obj);
   evas_object_image_file_set(sd->img, sd->async_file, sd->async_key);

   // temp code for failure of file_set
   sd->async_error = evas_object_image_load_error_get(sd->img);
   if (sd->async_error  != EVAS_LOAD_ERROR_NONE)
     {
        ERR("file_set error !!(%p)", obj);
        goto end;
     }
   ...
   if (sd->async_error  != EVAS_LOAD_ERROR_NONE)
     {
        ERR("Things are going bad for '%s' (%p)", sd->async_file, sd->img);
        goto end;
     }
   ...
   evas_object_smart_callback_call(obj, SIG_ASYNC_FILE_SET_ERROR, &sd->async_error);
...
_elm_image_async_open_cancel(void *data, Ecore_Thread *thread EINA_UNUSED)
{
   ...
   // Should we do anything special if open was canceled?
                         ...
                         const char *group)
{
   ...
   if (!edje_object_file_set(sd->img, file, group))
     {
     	// Tizen -> remove logs, after , need to be changed to secure logs
                          ...
                          const char *key)
{
   ...
   if (!sd->async)
     ...
     {
        // TODO: Check that (file,key) != (sd->async_file,sd->async_key)
        if (sd->async_th)
          {
             ecore_thread_cancel(sd->async_th);
             sd->async_th = NULL;
          }
        eina_stringshare_replace(&sd->async_file, file);
        eina_stringshare_replace(&sd->async_key, key);

        evas_object_ref(obj); // prevent premature death
        sd->async_th = ecore_thread_run(_elm_image_async_open_do,
                                        _elm_image_async_open_done,
                                        _elm_image_async_open_cancel,
                                        obj);
     }
...
EAPI void
elm_image_async_open_set(Evas_Object *obj, Eina_Bool async)
{
   ELM_IMAGE_CHECK(obj);
   ELM_IMAGE_DATA_GET(obj, sd);

   if (sd->async == async) return;
   sd->async = async;
   ...
   if (!async)
     {
        if (sd->async_th)
          ecore_thread_cancel(sd->async_th);
        // TODO: add a flag "opening" to sd and,
        // if it's set, do the open synchronously here
     }
}

EAPI Eina_Bool
elm_image_async_open_get(Evas_Object *obj)
{
   ELM_IMAGE_CHECK(obj) EINA_FALSE;
   ELM_IMAGE_DATA_GET(obj, sd);
