   ...
   if (sd->indicator_format_func)
     {
        ...
        eina_strbuf_append(buf, E_(" of "));//[string:error] not included in po file.
...
#else
        eina_strbuf_append_printf(buf, "%d %s 100", (int)round(per), E_(" of "));//[string:error] not included in po file.
#endif
     }
}
static void
...
elm_slider_add(Evas_Object *parent)
{
   ...
   //Tizen Only (140918)
   //indicator_show is now disabled by default,
   //and it will be handled by config value
   ...
   _elm_access_text_set
     (_elm_access_object_get(obj), ELM_ACCESS_TYPE, E_("IDS_TPLATFORM_BODY_SLIDER_T_TTS"));
