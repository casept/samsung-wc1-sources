...
static void
_transit_obj_state_free(Elm_Transit_Obj_State *state)
{
  if(!state)
    return;
...
_transit_obj_data_update(Elm_Transit *transit, Evas_Object *obj)
{
   ...
   if ((!transit->state_keep) && (obj_data->state))
     {
       _transit_obj_state_free(obj_data->state);
       obj_data->state = NULL;
     }
   else
     {
...
_transit_obj_remove_cb(void *data, Evas *e __UNUSED__, Evas_Object *obj, void *event_info __UNUSED__)
{
   ...
   Elm_Transit_Obj_Data *obj_data = evas_object_data_del(obj, _transit_key);
   if (obj_data)
     {
       _transit_obj_state_free(obj_data->state);
       free(obj_data);
...
_transit_obj_data_recover(Elm_Transit *transit, Evas_Object *obj)
{
   ...
   obj_data = evas_object_data_del(obj, _transit_key);
   if (!obj_data) return;
        ...
        if (!transit->state_keep)
          {
             ...
             evas_object_map_set(obj, state->map); // internally obj's map is realloced and copied form state->map
          }

        _transit_obj_state_free(state);
     }
   free(obj_data);
}
