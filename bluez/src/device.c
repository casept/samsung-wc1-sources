								...
								void *user_data)
{
	...
	if (!success) {
		...
		if (device->attachid == 0) {
			error("GATT client / server both are not connected. Disconnect all");
			disconnect_all(device);
			return;
		}
							...
							void *user_data)
{
	...
	/* Don't emit signal if reading all the characteristics in progress */
	if (btd_gatt_client_is_waiting_chrc_ready(device->client_dbus))
		return;
