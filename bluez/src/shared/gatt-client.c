...
#ifdef __TIZEN_PATCH__
static void notify_cancel(void *data)
{
	struct notify_data *notify_data = data;

	if (notify_data->att_id) {
		util_debug(notify_data->client->debug_callback,
				notify_data->client->debug_data,
				"Cancel notify req [att_id : %d]", notify_data->att_id);
...
static void gatt_client_cancel_and_remove_all_notify_in_range(
				struct bt_gatt_client *client,
...
#ifdef __TIZEN_PATCH__
	gatt_client_cancel_and_remove_all_notify_in_range(client, start_handle,
								end_handle);
#else
