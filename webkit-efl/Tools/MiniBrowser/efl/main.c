...
static Eina_Bool pasteboard_allowed = EINA_FALSE;
...
unsigned short webinspector_port = 0u;
Eina_Bool webinspector_started = EINA_FALSE;
...
static const Ecore_Getopt options = {
    ...
    EINA_TRUE, {
        ...
        ECORE_GETOPT_STORE_DEF_BOOL
            ('a', "allow-pasteboard", "Allow for JavaScript access to clipboard", EINA_FALSE),
        ECORE_GETOPT_STORE_DEF_BOOL
        ...
        ECORE_GETOPT_STORE_METAVAR_USHORT
            ('W', "webinspector", "Run WebInspector server on specified port.", "port-number"),
...
static Eina_Bool on_orientation_lock(Ewk_View_Smart_Data *sd, int orientation)
{
    const char* orientationString = "";
    if (orientation & EWK_SCREEN_ORIENTATION_PORTRAIT_PRIMARY)
        orientationString = "portrait(primary)";
    else if (orientation & EWK_SCREEN_ORIENTATION_LANDSCAPE_PRIMARY)
        orientationString = "landscape(primary)";

    printf("orientation lock is called(%s)\n", orientationString);
    return EINA_TRUE;
}

static void on_orientation_unlock(Ewk_View_Smart_Data *sd)
{
    printf("orientation unlock is called\n");
}
...
static Browser_Window *window_create(Evas_Object *opener, int width, int height, Eina_Bool view_mode)
{
    ...
    ewkViewClass->orientation_lock = on_orientation_lock;
    ewkViewClass->orientation_unlock = on_orientation_unlock;
    ...
    ewk_settings_dom_paste_allowed_set(settings, pasteboard_allowed);
    ewk_settings_javascript_can_access_clipboard_set(settings, pasteboard_allowed);
    ...
    if (webinspector_port > 0 && !webinspector_started) {
        unsigned new_webinspector_port;
        if (new_webinspector_port = ewk_view_inspector_server_start(window->ewk_view, webinspector_port)) {
            webinspector_port = (unsigned short)(new_webinspector_port);

            info("WebInspector server listening on port %hu", webinspector_port);
            webinspector_started = EINA_TRUE;
        } else {
            Eina_Stringshare* msg = eina_stringshare_printf("ERROR: Cannot open listening socket for WebInspector server on port %hu.\n", webinspector_port);
            quit(EINA_FALSE, msg);
