...
PageViewportController::PageViewportController(WebKit::WebPageProxy* proxy, PageViewportControllerClient* client)
    ...
    , m_pageTransitionViewportReady(false)
    , m_didNotifyReadyState(false)
#endif
{
...
#if ENABLE(TIZEN_NOTIFY_FIRST_LAYOUT_COMPLETE)
    if (!m_didNotifyReadyState && m_pageTransitionViewportReady && (m_webPageProxy->estimatedProgress() > 0.1)) {
        m_client->didFirstFrameReady();
        m_didNotifyReadyState = true;
    }
#endif
}
...
{
#if ENABLE(TIZEN_NOTIFY_FIRST_LAYOUT_COMPLETE)
    m_pageTransitionViewportReady = true;
#endif
