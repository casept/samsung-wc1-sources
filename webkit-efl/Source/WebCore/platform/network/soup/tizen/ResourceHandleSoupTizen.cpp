...
static void ensureSessionIsInitialized(SoupSession* session)
{
    ...
    static const int defaultTimeout = 60; // seconds
#endif // ENABLE(TIZEN_TIMEOUT_FIX)
...
static void gotHeadersCallback(SoupMessage* message, gpointer data)
{
...
#if PLATFORM(EFL) && USE(SOUP) && ENABLE(TIZEN_CERTIFICATE_HANDLING)
    ...
    if (handle) {
        ...
        if (WebCore::protocolIs(String(uri), "https")) { // only for https sites
            ...
            g_object_get(message, SOUP_MESSAGE_TLS_CERTIFICATE, &certificate, SOUP_MESSAGE_TLS_ERRORS, &errors, NULL); // get peer Certificate for error case
            if (certificate) {
...
#if !ENABLE(TIZEN_SOUP_AVOID_CERTIFICATION_POPUP_WORKAROUND)
...
#endif /* PLATFORM(EFL) && USE(SOUP) && ENABLE(TIZEN_CERTIFICATE_HANDLING */
...
static void doRedirect(ResourceHandle* handle)
{
    ...
    if (!newURL.protocolIs("http") && !newURL.protocolIs("https"))
        d->client()->didReceiveSchemeChallenge(newRequest);
#endif
    ...
    if (error) {
        ...
        if (d->m_soupMessage && SOUP_STATUS_IS_SERVER_ERROR(soupMessage->status_code)) {
            if (!(soupMessage->response_body) || (soupMessage->response_body->length == 0)) {
                    ...
                    cleanupSoupRequestOperation(handle.get());
                    handle->client()->didFail(handle.get(), resourceError);
                    return;
            }
        }
#endif // ENABLE(TIZEN_SOUP_HANDLE_STATUS_CODE_5XX_AFTER_RECEIVING_HTTP_BODY)
...
#endif // ENABLE(TIZEN_SOUP_NOT_ALLOW_BROWSE_LOCAL_DIRECTORY)
...
#endif // ENABLE(TIZEN_HTTP_REQUEST_HEADER_APPEND)
