...
#if ENABLE(TIZEN_SET_PRIORITY_TO_USER_SETTING_LANGUAGE)
#include <locale.h>
#endif
...
#if ENABLE(TIZEN_FALLBACK_FONT_WEIGHT)
FcPattern* createFontConfigPatternForCharacters(const FontDescription& description, const UChar* characters, int bufferLength)
#else
...
#if ENABLE(TIZEN_SET_PRIORITY_TO_USER_SETTING_LANGUAGE)
    //RFC3066 formatted 2-level language identifier will be returned.
    //e.g., en-NR, ar-SD, byn, tig etc.
    //But fontconfig only need 1-level language identifier having maximum 3 letters.
    char* fullLang = static_cast<char*>(setlocale(0, NULL));
    if (fullLang != NULL) {
        char lang[4] = {0,};
        strncpy(lang, fullLang, 3);
        for (int i = 0; i < 3; i++) {
            if (lang[i] == '-' || lang[i] == '_') {
                lang[i] = 0;
                break;
        ...
        if (!FcPatternAddString(pattern, FC_LANG, reinterpret_cast<FcChar8*>(lang)))
            return 0;
    }
#endif
...
#if ENABLE(TIZEN_FALLBACK_FONT_WEIGHT)
    if (!FcPatternAddInteger(pattern, FC_WEIGHT, fontWeightToFontconfigWeight(description.weight())))
        return 0;
#endif
...
{
#if ENABLE(TIZEN_FALLBACK_FONT_WEIGHT)
    UNUSED_PARAM(originalFontData);
    RefPtr<FcPattern> pattern = adoptRef(createFontConfigPatternForCharacters(description, characters, length));
#else
...
PassOwnPtr<FontPlatformData> FontCache::createFontPlatformData(const FontDescription& fontDescription, const AtomicString& family)
{
    ...
    if (fullLang != NULL) {
        ...
        if (!FcPatternAddString(pattern.get(), FC_LANG, reinterpret_cast<FcChar8*>(lang)))
            return nullptr;
    }
#endif
