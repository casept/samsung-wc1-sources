...
public:
    ...
    int offsetForPosition(const TextRun&, float position, bool includePartialGlyphs, bool forEllipsis = false) const;
#else
...
#if ENABLE(TIZEN_APPLY_COMPLEX_TEXT_ELLIPSIS)
    int offsetForPositionForComplexText(const TextRun&, float position, bool includePartialGlyphs, bool forEllipsis = false) const;
#else
