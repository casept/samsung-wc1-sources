...
bool GraphicsContext3D::ImageExtractor::extractImage(bool premultiplyAlpha, bool ignoreGammaAndColorProfile)
{
    ...
    if (m_image->filenameExtension() == "jpg" && decoder.size().width() * decoder.size().height() > static_cast<int>(decoder.maxPixelsPerDecodedImage())) {
        float scaleX = static_cast<float>(decoder.size().width()) / static_cast<float>(m_imageWidth);
        float scaleY = static_cast<float>(decoder.size().height()) / static_cast<float>(m_imageHeight);

        RefPtr<cairo_surface_t> expand_image = adoptRef(cairo_image_surface_create(cairo_image_surface_get_format(m_imageSurface.get()),
        ...
        cairo_set_source_surface(expand_cr.get(), m_imageSurface.get(), 0, 0);
        cairo_set_operator(expand_cr.get(), CAIRO_OPERATOR_SOURCE);
        cairo_paint(expand_cr.get());

        m_imageSurface = expand_image;
        m_imageWidth = cairo_image_surface_get_width(m_imageSurface.get());
        m_imageHeight = cairo_image_surface_get_height(m_imageSurface.get());
    }
#endif
