...
public:
    ...
    virtual ~CoordinatedBackingStoreTile() { }
#endif
...
public:
#if USE(TIZEN_PLATFORM_SURFACE)
    virtual ~CoordinatedBackingStore() { }

    virtual void createTile(uint32_t tileID, float);
    virtual void removeTile(uint32_t tileID);
    virtual void removeAllTiles();
    virtual bool isBackedBySharedPlatformSurface() { return false; }
#else
