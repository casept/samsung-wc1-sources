...
void CoordinatedGraphicsLayer::recalculateDirtyRectIfNeeded()
{
    // FIXME : Need to optimize.
    // Current dirty area is calculated based on previous m_offsetFromRenderer value.
    // So we need to update dirty rect after updating m_offsetFromRenderer if dirty rect is not drawn yet.
    if (m_mainBackingStore && m_mainBackingStore->hasDirtyRect())
        setNeedsDisplay();
}
#endif
...
static inline bool setSnapOffsetsAsFloat(Vector<float>& stateSnapOffsets, const Vector<LayoutUnit>& snapOffsets)
{
    Vector<float> snapOffsetsAsFloat;
    snapOffsetsAsFloat.reserveInitialCapacity(snapOffsets.size());

    bool isEqual = (stateSnapOffsets.size() == snapOffsets.size());
    for (size_t i = 0; i < snapOffsets.size(); ++i) {
        float value = snapOffsets[i];
        if (isEqual && (stateSnapOffsets[i] != value))
            isEqual = false;
        snapOffsetsAsFloat.append(value);
    }

    if (isEqual)
        return false;

    stateSnapOffsets = snapOffsetsAsFloat;
    return true;
}

void CoordinatedGraphicsLayer::setHorizontalSnapOffsets(const Vector<LayoutUnit>* offsets)
{
    if (!offsets) {
        if (m_layerState.horizontalSnapOffsets.isEmpty())
            return;
        m_layerState.horizontalSnapOffsets.clear();
    } else {
        if (!setSnapOffsetsAsFloat(m_layerState.horizontalSnapOffsets, *offsets))
            return;
    }

    m_layerState.horizontalSnapOffsetsChanged = true;
    didChangeLayerState();
}

void CoordinatedGraphicsLayer::setVerticalSnapOffsets(const Vector<LayoutUnit>* offsets)
{
    if (!offsets) {
        if (m_layerState.verticalSnapOffsets.isEmpty())
            return;
        m_layerState.verticalSnapOffsets.clear();
    } else {
        if (!setSnapOffsetsAsFloat(m_layerState.verticalSnapOffsets, *offsets))
            return;
    }

    m_layerState.verticalSnapOffsetsChanged = true;
    didChangeLayerState();
}
#endif
