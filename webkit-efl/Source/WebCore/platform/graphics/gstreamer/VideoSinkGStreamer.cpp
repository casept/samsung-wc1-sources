...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
static gboolean webkit_video_sink_event(GstBaseSink* base_sink, GstEvent* event)
{
    gboolean ret = TRUE;
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM_OOB:
        if (structure) {
            ...
            GST_BUFFER_MALLOCDATA(buf) = (guint8*) g_malloc(sizeof(addr));
            GST_BUFFER_DATA(buf) = GST_BUFFER_MALLOCDATA(buf);
            gst_buffer_set_data(buf, (guint8*) addr, sizeof(addr));
