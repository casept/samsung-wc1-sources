...
public:
    ...
    virtual PlatformLayer* platformLayer() const { return m_videoLayer->platformLayer(); }
    virtual bool supportsAcceleratedRendering() const { return true; }
    virtual void paintCurrentFrameInContext(GraphicsContext* context, const IntRect& rect) { m_videoLayer->paintCurrentFrameInContext(context, rect); }
#endif
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    virtual bool isLocalMediaStream() const = 0;
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
#if ENABLE(TIZEN_MSE)
    virtual MediaPlayer::ReadyState mediasourceReadyState() const = 0;
