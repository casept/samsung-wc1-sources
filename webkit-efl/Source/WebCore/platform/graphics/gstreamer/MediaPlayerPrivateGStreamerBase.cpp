...
#endif
#if USE(TIZEN_PLATFORM_SURFACE)
    , m_videoLayer(VideoLayerTizen::create(player->mediaPlayerClient()))
#endif
{
    TIZEN_LOGI("%p", this);
...
IntSize MediaPlayerPrivateGStreamerBase::naturalSize() const
{
    ...
    GRefPtr<GstCaps> caps = webkitGstGetPadCaps(videoSinkPad.get());
#endif

    if (!caps) {
#if ENABLE(TIZEN_MEDIA_STREAM)
        if (isLocalMediaStream())
            return IntSize(LocalMediaServer::instance().width(), LocalMediaServer::instance().height());
#endif
        return IntSize();
    }
...
{
    TIZEN_LOGI("%d", m_muteSignalHandler);
...
GstElement* MediaPlayerPrivateGStreamerBase::createVideoSink(GstElement* pipeline)
{
    ...
    // Don't need to actual video sink for audio element.
    if (!m_player->mediaPlayerClient()->isVideoElement()) {
    ...
    // Screen mirroring option. Primary video should be OFF and Secondary video should be FULL SCREEN mode
    g_object_set(m_webkitVideoSink.get(), "display-mode", 2, NULL);

    // Don't fill empty area with borders
    g_object_set(m_webkitVideoSink.get(), "draw-borders", false, NULL);

    // Don't generate qos message
    g_object_set(m_webkitVideoSink.get(), "qos", false, NULL);

    return m_webkitVideoSink.get();
#endif
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    // Don't create unnecessary 'fpsdisplaysink'.
    return m_webkitVideoSink.get();
