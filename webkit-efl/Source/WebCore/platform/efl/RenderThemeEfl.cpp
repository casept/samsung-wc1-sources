...
static const int sliderThumbWidth = 46;
static const int sliderThumbHeight = 46;
#else
...
#if ENABLE(TIZEN_WEARABLE_PROFILE)
static const int mediaSliderHeight = 6;
static const int mediaSliderFullScreenThumbWidth = 20;
static const int mediaSliderFullScreenThumbHeight = 20;
#else // ENABLE(TIZEN_WEARABLE_PROFILE)
static const int mediaSliderHeight = 4;
static const int mediaSliderFullScreenThumbWidth = 30;
static const int mediaSliderFullScreenThumbHeight = 30;
#endif // ENABLE(TIZEN_WEARABLE_PROFILE)
static const int mediaSliderThumbWidth = 14;
...
bool RenderThemeEfl::paintThemePart(RenderObject* object, FormType type, const PaintInfo& info, const IntRect& rect)
{
...
#if ENABLE(TIZEN_CORE_THEME_SCALE)
    ...
    if (type <= SliderThumbHorizontal) {
        cairo_get_matrix(cairo, &matrix);

        float themeScaleValue = m_partDescs[type].scale;
        ASSERT(themeScaleValue != 0.0);

        FloatSize scaledEntrySize(rect.size());
        scaledEntrySize.scale(matrix.xx, matrix.yy);

        entry = getThemePartFromCache(type, IntSize(scaledEntrySize.width(), scaledEntrySize.height()));
        if (!entry)
            return false;
        edje_object_scale_set(entry->edje(), matrix.xx / themeScaleValue);
    } else {
        entry = getThemePartFromCache(type, rect.size());
        if (!entry)
            return false;
        edje_object_scale_set(entry->edje(), 1);
    }
#else
    entry = getThemePartFromCache(type, rect.size());
#endif
...
#if ENABLE(TIZEN_WEARABLE_PROFILE)
    , m_mediaSliderColor(0x3C, 0x3C, 0x3C, 0x80)
    , m_mediaBufferingColor(0x8B, 0x5A, 0x00, 0x80)
    , m_mediaPlayingColor(0xFF, 0xA5, 0x00)
#else
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
        if(element && (element->document()->webkitIsFullScreen() || element->document()->isMediaDocument())){
            style->setWidth(Length(mediaSliderFullScreenThumbWidth, Fixed));
            style->setHeight(Length(mediaSliderFullScreenThumbWidth, Fixed));
            return;
        }
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
...
{
#if ENABLE(TIZEN_WEARABLE_PROFILE)
    return String(mediaControlsTizenMicroUserAgentStyleSheet, sizeof(mediaControlsTizenMicroUserAgentStyleSheet));
#else
...
bool RenderThemeEfl::paintMediaSliderTrack(RenderObject* object, const PaintInfo& info, const IntRect& rect)
{
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    int thumbWidth;
    if (mediaElement->isFullscreen() || mediaElement->ownerDocument()->isMediaDocument())
        thumbWidth = mediaSliderFullScreenThumbWidth;
    else
        thumbWidth = mediaSliderThumbWidth;
    ...
    context->drawLine(IntPoint(rect.x() + thumbWidth / 2 , rect.y() + rect.height() / 2), IntPoint(rect.x() + rect.width() - thumbWidth / 2, rect.y() + rect.height() / 2));
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    trackRect.setX(rect.x() + thumbWidth/2);
    totalWidth = trackRect.width() - thumbWidth;
    trackRect.setY(rect.y() + rect.height() / 2);
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
    context->save();
    ...
    for (unsigned index = 0; index < timeRanges->length(); ++index) {
        ...
        if (!index) {
            ...
            if (suspendedPoint != start) {
                ...
                width = ((end - start) * totalWidth) / mediaDuration;
            }
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
        suspendedPoint = timeRanges->end(index, IGNORE_EXCEPTION);
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
        // In case of some files, metadata is positioned at end of file.
        // Don't show this.
        if (timeRanges->length() > 1 && index == timeRanges->length() - 1)
            break;

        context->setLineCap(RoundCap);
        context->setStrokeStyle(SolidStroke);
        context->setStrokeColor(m_mediaBufferingColor, ColorSpaceDeviceRGB);
        context->setStrokeThickness(rangeRect.height());
        context->drawLine(IntPoint(rangeRect.x(), rangeRect.y()), IntPoint(rangeRect.x() + rangeRect.width(), rangeRect.y()));
#else
    ...
    if (width > 0) {
        ...
        context->drawLine(IntPoint(trackRect.x(), trackRect.y()), IntPoint(trackRect.x() + width, trackRect.y()));
    }
#endif // ENABLE(TIZEN_GSTREAMER_VIDEO)
    context->restore();
    return true;
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
bool RenderThemeEfl::paintMediaTimeRemaining(RenderObject*, const PaintInfo& info, const IntRect& rect)
{
    info.context->fillRect(FloatRect(rect), m_mediaPanelColor, ColorSpaceDeviceRGB);
    return true;
}
