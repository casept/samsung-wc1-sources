...
static Length convertToIntLength(const CSSPrimitiveValue* primitiveValue, const RenderStyle* style, const RenderStyle* rootStyle, double multiplier)
{
...
{
    return value.convertToLength<FixedIntegerConversion | PercentConversion | AutoConversion>(m_state.style(), m_state.rootElementStyle(), m_state.style()->effectiveZoom());
}

Length StyleResolver::parseSnapCoordinate(CSSValueList& valueList, unsigned offset)
{
    return valueList.item(offset)->isPrimitiveValue() ? parseSnapCoordinate(*static_cast<CSSPrimitiveValue*>(valueList.item(offset))) : Length();
}
...
PassOwnPtr<ScrollSnapPoints> StyleResolver::parseSnapPoints(CSSValue& value)
{
    OwnPtr<ScrollSnapPoints> points = adoptPtr(new ScrollSnapPoints());

    if (value.isPrimitiveValue() && static_cast<CSSPrimitiveValue&>(value).getValueID() == CSSValueElements) {
    ...
    for (CSSValueListIterator it(&value); it.hasMore(); it.advance()) {
        if (!it.value()->isPrimitiveValue())
            continue;
        auto& itemValue = *static_cast<CSSPrimitiveValue*>(it.value());
...
void StyleResolver::applyProperty(CSSPropertyID id, CSSValue* value)
{
    ...
    case CSSPropertyWebkitScrollSnapPointsX:
        if (isInherit) {
            m_state.style()->setScrollSnapPointsX(m_state.parentStyle()->scrollSnapPointsX() ? adoptPtr(new ScrollSnapPoints(*m_state.parentStyle()->scrollSnapPointsX())) : nullptr);
            return;
        }
        if (isInitial) {
            m_state.style()->setScrollSnapPointsX(nullptr);
            return;
        }
    ...
    case CSSPropertyWebkitScrollSnapPointsY:
        if (isInherit) {
            m_state.style()->setScrollSnapPointsY(m_state.parentStyle()->scrollSnapPointsY() ? adoptPtr(new ScrollSnapPoints(*m_state.parentStyle()->scrollSnapPointsY())) : nullptr);
            return;
        }
        if (isInitial) {
            m_state.style()->setScrollSnapPointsY(nullptr);
            return;
        }
    ...
    case CSSPropertyWebkitScrollSnapDestination: {
        ...
        if (!value->isValueList())
            return;
        state.style()->setScrollSnapDestination(parseSnapCoordinatePair(*static_cast<CSSValueList*>(value), 0));
        return;
    }
