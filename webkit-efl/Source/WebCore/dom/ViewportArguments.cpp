...
#if ENABLE(TIZEN_EXCLUDE_HEIGHT_FROM_MINIMUM_SCALE_CALC_WORKAROUND)
    // When IME is shown, webview is resized to be scrolled on IME. In that case, WK2 re-calculates
    // viewport size, initial scale value is calculated by viewport size and content size.
    // Width doesn't influence on the initial scale. However, height can change the scale value when
    // content height still keeps shotten height though viewport size restores original height.
    // In Tizen, we only uses width value for calculating minimum scale.
