...
#if ENABLE(TIZEN_SCRIPTED_SPEECH)
    if (hidden()) {
        SpeechRecognitionController* controller = SpeechRecognitionController::from(m_frame->page());
        if (controller)
            controller->client()->stopWebSpeechOnBackgroundMode();
    }
#endif
...
#else // ENABLE(TIZEN_EXTENSIBLE_API)
        if (!ScriptController::processingUserGesture())
            break;
#endif // ENABLE(TIZEN_EXTENSIBLE_API)
...
void Document::removeListenerType(const AtomicString& eventType)
{
...
#endif // ENABLE(TIZEN_PLUGIN_TOUCH_EVENT)
