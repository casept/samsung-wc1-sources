...
ALWAYS_INLINE RenderStyle::RenderStyle(bool)
{
    ...
    rareNonInheritedData.access()->m_scrollSnapPoints.init();
#endif
...
void RenderStyle::setScrollSnapPointsX(PassOwnPtr<ScrollSnapPoints> points)
{
    if (rareNonInheritedData->m_scrollSnapPoints->xPoints.get() == points.get())
        return;
    rareNonInheritedData.access()->m_scrollSnapPoints.access()->xPoints = points;
}

void RenderStyle::setScrollSnapPointsY(PassOwnPtr<ScrollSnapPoints> points)
{
    if (rareNonInheritedData->m_scrollSnapPoints->yPoints.get() == points.get())
        return;
    rareNonInheritedData.access()->m_scrollSnapPoints.access()->yPoints = points;
}

void RenderStyle::setScrollSnapDestination(const LengthSize& destination)
{
    if (rareNonInheritedData->m_scrollSnapPoints->destination == destination)
        return;
    rareNonInheritedData.access()->m_scrollSnapPoints.access()->destination = destination;
}

void RenderStyle::setScrollSnapCoordinates(const Vector<LengthSize>& coordinates)
{
    if (rareNonInheritedData->m_scrollSnapPoints->coordinates == coordinates)
        return;
    rareNonInheritedData.access()->m_scrollSnapPoints.access()->coordinates = coordinates;
}
