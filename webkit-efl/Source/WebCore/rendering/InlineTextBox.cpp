...
float InlineTextBox::placeEllipsisBox(bool flowIsLTR, float visibleLeftEdge, float visibleRightEdge, float ellipsisWidth, float &truncatedWidth, bool& foundBox)
{
    ...
    if (ltrEllipsisWithinBox || rtlEllipsisWithinBox) {
        ...
        int offset = offsetForPosition(ellipsisX, false, true);
#else
...
void InlineTextBox::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset, LayoutUnit /*lineTop*/, LayoutUnit /*lineBottom*/)
{
    ...
    if (paintInfo.phase != PaintPhaseSelection && paintInfo.phase != PaintPhaseTextClip && !isPrinting) {
        ...
        if (containsComposition && !useCustomUnderlines)
        ...
        else if (containsComposition && useCustomUnderlines) {
            ...
            for (size_t index = 0; index < numUnderlines; ++index) {
                ...
                if (underline.startOffset <= end()) {
                    // underline intersects this run.  Paint the background of the underline.
                    paintCompositionUnderlineBackground(context, boxOrigin, styleToUse, font, underline);
...
#if ENABLE(TIZEN_APPLY_COMPLEX_TEXT_ELLIPSIS)
int InlineTextBox::offsetForPosition(float lineOffset, bool includePartialGlyphs, bool forEllipsis) const
#else
...
#if ENABLE(TIZEN_APPLY_COMPLEX_TEXT_ELLIPSIS)
    return font.offsetForPosition(constructTextRun(style, font), lineOffset - logicalLeft(), includePartialGlyphs, forEllipsis);
#else
...
#if ENABLE(TIZEN_WEBKIT2_SUPPORT_JAPANESE_IME)
void InlineTextBox::paintCompositionUnderlineBackground(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, const CompositionUnderline underline)
{
    int offset = m_start;
    int sOffset = static_cast<int>(underline.startOffset);
    int eOffset = static_cast<int>(underline.endOffset);
    int length = static_cast<int>(m_len);
    int sPos = max(sOffset - offset, 0);
    int ePos = min(eOffset - offset, length);
    ...
    Color c = underline.backgroundColor;
    if (!c.isValid())
        return;
