...
#if ENABLE(TIZEN_MYANMAR_FONT_SUPPORT)
#include "Language.h"
#endif
...
#if ENABLE(TIZEN_MYANMAR_FONT_SUPPORT)
static String handleMyanmarFont(String m_text)
{
    StringBuilder result;
    String resultString;
    result.reserveCapacity(m_text.length());
    for (size_t i = 0; i < m_text.length(); ++i) {
        if (0x1000 <= m_text[i] && m_text[i] <= 0x109F)
            result.append(m_text[i] | 0xF100);
        else
            result.append(m_text[i]);
    }
    if (result.length())
    ...
    , m_lastTextBox(0)
{
    ...
    if (!m_text.isEmpty() && (locale == "my-ZG")) {
        String convertedString = handleMyanmarFont(m_text);
        m_text = convertedString.impl();
    }
#endif
