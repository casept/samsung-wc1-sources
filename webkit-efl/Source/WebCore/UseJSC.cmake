...
if (ENABLE_GAMEPAD AND ("${TIZEN_PROFILE}" STREQUAL "TV"))
    list(APPEND WebCore_SOURCES
        bindings/js/JSGamepadListCustom.cpp
    )
endif ()
...
# FIXME: (Tizen specific) Tizen needs to have below change in order to build Tizen WebKit with SHARED_CORE=ON"
set (WebCore_SOURCES_GEN)
GENERATE_BINDINGS(WebCore_SOURCES_GEN
    "${WebCore_IDL_FILES}"
    "${WEBCORE_DIR}"
    "${IDL_INCLUDES}"
...
# FIXME: (TizenSpecific) Tizen needs to have below change in order to build Tizen WebKit with SHARED_CORE=ON"
if (ENABLE_TIZEN_ALL_IN_ONE)
    # This will put all DerivedSources/*.cpp into one big cpp and greatly reduce linking command length.
    set (IGNORE_LIST
    # Put your files to be ignored here:
    )
    set (ALLINONE_SOURCES)
    list (REMOVE_DUPLICATES WebCore_SOURCES_GEN)
    foreach (_file ${WebCore_SOURCES_GEN})
        get_filename_component (_ext ${_file} EXT)

        list (FIND IGNORE_LIST ${_file} RESULT)
        if (${_ext} STREQUAL ".cpp" AND RESULT EQUAL -1)
            set (ALLINONESTR "${ALLINONESTR}\n#include \"${_file}\"")
            set_property (SOURCE ${DERIVED_SOURCES_WEBCORE_DIR}/AllInOneEfl.cpp APPEND PROPERTY OBJECT_DEPENDS ${_file})
        else ()
            list (APPEND ALLINONE_SOURCES ${_file})
        endif ()
    endforeach ()
    WRITE_FILE_IF_CHANGED(${DERIVED_SOURCES_WEBCORE_DIR}/AllInOneEfl.cpp ${ALLINONESTR})
    list (APPEND ALLINONE_SOURCES ${DERIVED_SOURCES_WEBCORE_DIR}/AllInOneEfl.cpp)

    set (WebCore_SOURCES ${WebCore_SOURCES} ${ALLINONE_SOURCES})
else ()
    set (WebCore_SOURCES ${WebCore_SOURCES} ${WebCore_SOURCES_GEN})
endif () # if (SHARED_CORE)
