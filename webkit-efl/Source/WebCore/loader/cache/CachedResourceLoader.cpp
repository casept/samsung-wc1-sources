...
#if ENABLE(TIZEN_CACHE_RESOURCE_FILTER)
    if (type == CachedResource::MainResource && url.protocol() == String("file") && (url.host().isNull() || url.host().isEmpty())) {
        StringBuilder builder;
        builder.append('/');
        builder.append(url.lastPathComponent());
        if (url.path() == builder.toString())
            return resource;
    }
#endif
...
void CachedResourceLoader::setLoadRemoteImages(bool loadRemoteImages)
{
    ...
    for (DocumentResourceMap::iterator it = m_documentResources.begin(); it != end; ++it) {
        CachedResource* resource = it->value.get();
        if (resource->type() == CachedResource::ImageResource && resource->stillNeedsLoad())
            const_cast<CachedResource*>(resource)->load(this, defaultCachedResourceOptions());
    }
}
#endif // ENABLE(TIZEN_LOAD_REMOTE_IMAGES)
