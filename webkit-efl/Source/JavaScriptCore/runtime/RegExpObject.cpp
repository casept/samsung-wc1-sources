...
bool RegExpObject::test(ExecState* exec, JSString* string)
{
    ...
    MatchResult result;
    if (regExp()->lookupTestCache(string, result)) {
        if (result)
            exec->lexicalGlobalObject()->regExpConstructor()->recordCachedResult(exec->vm(), regExp(), string, result);
        return result;
    }
    result = match(exec, string);
    regExp()->createTestCache(string, result);
