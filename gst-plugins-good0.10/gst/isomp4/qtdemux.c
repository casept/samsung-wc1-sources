    ...
    GstByteReader * parser)
{
  ...
  for (child = g_node_first_child (node); child;
    ...
    if(child_fourcc == 0 && child_len >= 8) {   // in case when tkhd is missing (PLM : P140519-04349)
        child_fourcc = fourcc;                         // It will still show the pop-up as "Unsupported Video Format" since the MPEG4 is NOT supported for gear devices and the attached videio in the PLM is an MPEG 4 content
    }
    ...
    if(stream->fourcc == FOURCC_mp4v) {
      ...
      for(i=0; i<data_len-4; i++) {
        if(QT_UINT32(data_ptr+i) == 0x00000120) {
          GST_WARNING_OBJECT (qtdemux, "Found the VOL Marker in DCI Info, It is MPEG-4 Content");
          err = FALSE;
          break;
        }
      }
      if(err) {
        GST_WARNING_OBJECT (qtdemux, "Not Found the VOL Marker in DCI Info. but caps is not changed to H263.");
/*Fix demuxer try to link h263 decoder to mpeg4 file.*/
