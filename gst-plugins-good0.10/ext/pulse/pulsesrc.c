...
#ifdef __TIZEN__
#define DEFAULT_AUDIO_LATENCY   PULSESRC_LATENCY_MID
#define AUDIO_READ_LOG_COUNT_MAX    50
#endif
...
#ifdef __TIZEN__
#define GST_PULSESRC_LATENCY      (gst_pulsesrc_audio_latency_get_type())
#endif
...
#ifdef __TIZEN__
#define PULSESRC_LOW_LATENCY                75000
#define PULSESRC_MID_LATENCY                150000
#define PULSESRC_HIGH_LATENCY               200000
#endif
...
#define GST_PULSESRC_DUMP_INPUT_PATH_PREFIX    "/tmp/dump_pulsesrc_in"
#define GST_PULSESRC_DUMP_OUTPUT_PATH_PREFIX   "/tmp/dump_pulsesrc_out"
#define GST_PULSESRC_DUMP_INPUT_FLAG           0x00200000
#endif
#endif
...
#ifdef __TIZEN__
static void
gst_pulsesrc_get_buffer_configuration (GstPulseSrc * pulsesrc)
{
  ...
  if (!dict) {
    GST_INFO("open failed. path(%s). use default latency", path);
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_LOW] = PULSESRC_LOW_LATENCY;
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_MID] = PULSESRC_MID_LATENCY;
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_HIGH] = PULSESRC_HIGH_LATENCY;

    GST_WARNING_OBJECT(pulsesrc, "use default latency low(%dms), mid(%dms), high(%dms)", PULSESRC_LOW_LATENCY,
      PULSESRC_MID_LATENCY, PULSESRC_HIGH_LATENCY);
  } else {
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_LOW] = iniparser_getint(dict, "pulsesrc:low_buffer_time", PULSESRC_LOW_LATENCY);
    pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_LOW] = iniparser_getint(dict, "pulsesrc:low_latency_time", 0);
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_MID] = iniparser_getint(dict, "pulsesrc:mid_buffer_time", PULSESRC_MID_LATENCY);
    pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_MID] = iniparser_getint(dict, "pulsesrc:mid_latency_time", 0);
    pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_HIGH] = iniparser_getint(dict, "pulsesrc:high_buffer_time", PULSESRC_HIGH_LATENCY);
    pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_HIGH] = iniparser_getint(dict, "pulsesrc:high_latency_time", 0);

    GST_WARNING_OBJECT(pulsesrc, "latency_time low(%dms), mid(%dms), high(%dms)", pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_LOW],
      pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_MID], pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_HIGH]);
  }
...
#ifdef __TIZEN__
GType
gst_pulsesrc_audio_latency_get_type (void)
{
  static GType capture_audio_latency_type = 0;
  static const GEnumValue capture_audio_latency[] = {
    {PULSESRC_LATENCY_LOW, "Set capture latency as low", "low"},
    {PULSESRC_LATENCY_MID, "Set capture latency as mid", "mid"},
    {PULSESRC_LATENCY_HIGH, "Set capture latency as high", "high"},
    {0, NULL, NULL},
  };

  if (!capture_audio_latency_type) {
          capture_audio_latency_type =
        g_enum_register_static ("GstPulseSrcLatency", capture_audio_latency);
  }
...
#ifdef PCM_DUMP_ENABLE
static gboolean
gst_pulsesrc_pad_dump_handler (GstPad *pad, GstBuffer *buffer, gpointer data)
{
  GstPulseSrc *pulsesrc = GST_PULSESRC_CAST (data);
  int ret = -1;

  GST_DEBUG_OBJECT (pulsesrc, "gst_pulsesrc_pad_dump_handler");

  if (pulsesrc->dump_fd_input)
      ret = fwrite(GST_BUFFER_DATA(buffer), 1, GST_BUFFER_SIZE(buffer), pulsesrc->dump_fd_input);
...
#ifdef __TIZEN__
  pulsesrc->support_type = 0;
  pulsesrc->latency = DEFAULT_AUDIO_LATENCY;
  pulsesrc->readcnt = -1;
  memset (pulsesrc->buffer_time, 0, sizeof(pulsesrc->buffer_time));
  memset (pulsesrc->latency_time, 0, sizeof(pulsesrc->latency_time));
  gst_pulsesrc_get_buffer_configuration (pulsesrc);

#ifdef PCM_DUMP_ENABLE
  if (vconf_get_int(GST_PULSESRC_DUMP_VCONF_KEY, &vconf_dump)) {
      GST_WARNING("vconf_get_int %s failed", GST_PULSESRC_DUMP_VCONF_KEY);
  }
  pulsesrc->need_dump_input = vconf_dump & GST_PULSESRC_DUMP_INPUT_FLAG ? TRUE : FALSE;
  pulsesrc->dump_fd_input = NULL;
  GST_DEBUG_OBJECT (pulsesrc, "Niranjan-outside--%d",pulsesrc->need_dump_input);
  if (pulsesrc->need_dump_input) {
      GST_DEBUG_OBJECT (pulsesrc, "Niranjan---%d",pulsesrc->need_dump_input);
      srcpad = gst_element_get_static_pad((GstElement *)pulsesrc, "src");
      gst_pad_add_buffer_probe (srcpad, G_CALLBACK (gst_pulsesrc_pad_dump_handler), pulsesrc);
  }
#endif
#endif
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_AUDIO_LATENCY:
      pulsesrc->latency = g_value_get_enum(value);

      if (pulsesrc->latency == PULSESRC_LATENCY_HIGH) {
        if (pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_HIGH])
          GST_BASE_AUDIO_SRC (pulsesrc)->buffer_time = pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_HIGH];
        if (pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_HIGH])
          GST_BASE_AUDIO_SRC (pulsesrc)->latency_time = pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_HIGH];
      } else if (pulsesrc->latency == PULSESRC_LATENCY_MID) {
        if (pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_MID])
          GST_BASE_AUDIO_SRC (pulsesrc)->buffer_time = pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_MID];
        if (pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_MID])
          GST_BASE_AUDIO_SRC (pulsesrc)->latency_time = pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_MID];
      } else {
        if (pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_LOW])
          GST_BASE_AUDIO_SRC (pulsesrc)->buffer_time = pulsesrc->buffer_time[PULSESRC_LOCAL_CONFIGURATION_LOW];
        if (pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_LOW])
          GST_BASE_AUDIO_SRC (pulsesrc)->latency_time = pulsesrc->latency_time[PULSESRC_LOCAL_CONFIGURATION_LOW];
      }

      GST_WARNING_OBJECT(pulsesrc,"buffer information : latency(%d), buffer_time(%lld), latency_time(%lld)", pulsesrc->latency,
        GST_BASE_AUDIO_SRC (pulsesrc)->buffer_time, GST_BASE_AUDIO_SRC (pulsesrc)->latency_time);
      break;
    case PROP_AUDIO_SUPPORT_TYPE:
      pulsesrc->support_type = g_value_get_int(value);
      GST_INFO_OBJECT(pulsesrc, "support type(%d)", pulsesrc->support_type);
      break;
#endif
#ifdef HAVE_PULSE_1_0
    case PROP_VOLUME:
...
#ifdef __TIZEN__
    case PROP_AUDIO_LATENCY:
      g_value_set_enum(value, pulsesrc->latency);
      break;
    case PROP_AUDIO_SUPPORT_TYPE:
      g_value_set_int(value, pulsesrc->support_type);
      break;
#endif
...
#ifdef __TIZEN__
    if(pulsesrc->readcnt == -1) {
      GST_WARNING_OBJECT(pulsesrc, "Start to read audio data. (%" G_GSIZE_FORMAT ") bytes", pulsesrc->read_buffer_length);
    } else {
      if(pulsesrc->readcnt > AUDIO_READ_LOG_COUNT_MAX) {
        GST_WARNING_OBJECT(pulsesrc, "Read audio data. (%" G_GSIZE_FORMAT ") bytes", pulsesrc->read_buffer_length);
        pulsesrc->readcnt = -1;
      }
    }
    pulsesrc->readcnt++;
...
#ifdef __TIZEN__
  spec.latency_time = GST_BASE_AUDIO_SRC (pulsesrc)->latency_time;
#else
...
#ifdef PCM_DUMP_ENABLE
  klass = (GEnumClass *)g_type_class_ref(gst_pulsesrc_audio_latency_get_type());

  if (pulsesrc->need_dump_input == TRUE && pulsesrc->dump_fd_input == NULL) {
      char *suffix , *dump_path;
      GDateTime *time = g_date_time_new_now_local();

      suffix = g_date_time_format(time, "%m%d_%H%M%S");
      dump_path = g_strdup_printf("%s_%s_%dch_%dhz_%s.pcm", GST_PULSESRC_DUMP_INPUT_PATH_PREFIX,
      g_enum_get_value(klass, pulsesrc->latency)->value_nick, spec.channels, spec.rate, suffix);

      pulsesrc->dump_fd_input = fopen(dump_path, "w+");
...
#ifdef __TIZEN__
  name = (pulsesrc->latency == PULSESRC_LATENCY_HIGH) ? "HIGH LATENCY Record Stream" : (pulsesrc->latency == PULSESRC_LATENCY_MID)? "MID LATENCY Record Stream": (pulsesrc->latency == PULSESRC_LATENCY_LOW)? "LOW LATENCY Record Stream" : "Record Stream";
#else
  name = "Record Stream";
#endif

#ifdef __TIZEN__
  gst_pulse_set_policy(pulsesrc->proplist, pulsesrc->support_type);
#endif
  if (pulsesrc->proplist) {
