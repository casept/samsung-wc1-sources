...
#define PULSESINK_FADIG_TIME                30000

#ifdef PCM_DUMP_ENABLE
#define GST_PULSESINK_DUMP_VCONF_KEY            "memory/private/sound/pcm_dump"
#define GST_PULSESINK_DUMP_INPUT_PATH_PREFIX    "/tmp/dump_pulsesink_in"
#define GST_PULSESINK_DUMP_OUTPUT_PATH_PREFIX   "/tmp/dump_pulsesink_out"
...
gst_pulseringbuffer_acquire (GstRingBuffer * buf, GstRingBufferSpec * spec)
{
...
#ifdef __TIZEN__
  klass = (GEnumClass *)g_type_class_ref(gst_pulsesink_latency_get_type());
#endif
...
#else
    name = g_enum_get_value(klass, psink->latency)->value_name;
  ...
  if (psink->need_dump_input == TRUE && psink->dump_fd_input == NULL) {
    ...
    dump_path = g_strdup_printf("%s_%s_%dch_%dhz_%s.pcm", GST_PULSESINK_DUMP_INPUT_PATH_PREFIX,
      g_enum_get_value(klass, psink->latency)->value_nick, pbuf->channels, spec->rate, suffix);
  ...
  policy = pa_proplist_gets(psink->proplist, PA_PROP_MEDIA_POLICY);

  if (policy == NULL || (policy && strlen(policy) == 0)) {
#ifdef HAVE_PULSE_1_0
  ...
  GST_WARNING_OBJECT(psink, "vol_type(%d), gain(%d), policy(%s), latency(%d), format(%d), width(%d), rate(%d)",
    vol, gain, policy != NULL ? policy : "NULL", psink->latency, spec->format, spec->width, spec->rate);
...
#ifdef __TIZEN__
  if (policy && strcmp (policy, "voip") == 0) {
...
#ifdef __TIZEN__
  GST_DEBUG_OBJECT (psink, "set unmute. wait (%d)usec", PULSESINK_FADIG_TIME);
  if (!psink->mute_set && !psink->mute) {
    gst_pulsesink_set_mute(psink, FALSE);
    if(!psink->once)
      usleep(PULSESINK_FADIG_TIME);
    else
      psink->once = FALSE;
  }
#endif
...
#ifdef __TIZEN__
  GST_DEBUG_OBJECT (psink, "set mute. wait (%d)usec", PULSESINK_FADIG_TIME);
  if (!psink->mute_set && !psink->mute) {
...
gst_pulsesink_latency_get_type (void)
{
	...
	static const GEnumValue avsysaudio_latency[] = {
		{PULSESINK_LATENCY_LOW, "pulsesink:low", "low"},
		{PULSESINK_LATENCY_MID, "pulsesink:mid", "mid"},
		{PULSESINK_LATENCY_HIGH, "pulsesink:high", "high"},
		{PULSESINK_LATENCY_VERY_HIGH, "pulsesink:veryhigh", "very-high"},
		{0, NULL, NULL},
	};
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_AUDIO_LATENCY: {
        ...
        } else if(nvalue == PULSESINK_LATENCY_MID) {
        ...
        } else if(nvalue == PULSESINK_LATENCY_LOW) {
            if(pulsesink->buffer_time[PULSESINK_LOCAL_CONFIGURATION_LOW])
                GST_BASE_AUDIO_SINK (pulsesink)->buffer_time = pulsesink->buffer_time[PULSESINK_LOCAL_CONFIGURATION_LOW];
            if(pulsesink->latency_time[PULSESINK_LOCAL_CONFIGURATION_LOW])
                GST_BASE_AUDIO_SINK (pulsesink)->latency_time = pulsesink->latency_time[PULSESINK_LOCAL_CONFIGURATION_LOW];
        }
