...
static void pa_sink_input_set_smooth_volume(pa_sink_input* i, pa_cvolume* src, pa_cvolume* dest)
{
    int k = 0;
    int frame_size = pa_frame_size(&i->sample_spec);
    ...
    pa_log_info("start_vol(%d) -> end_vol(%d), ch(%d), diff(%d) dur(%d), freq(%d), angle(%f), keep(%d)",
        src->values[0], dest->values[0], i->sample_spec.channels, dest->values[0]-src->values[0],
    ...
    switch (code) {
        ...
        case PA_SINK_INPUT_MESSAGE_SET_SOFT_VOLUME:
            ...
            if (!pa_cvolume_equal(&i->thread_info.soft_volume, &i->soft_volume)) {
                pa_log_info("set smooth volume");

                if(pa_atomic_load(&i->thread_info.smooth_requested) == 0)
                    pa_sink_input_set_smooth_volume(i, &i->thread_info.soft_volume, &i->soft_volume);
                else
                    pa_log_info("skip set_volume because of fading");
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
        {
            ...
            pa_log_info("set %s", i->muted ? "mute" : "unmute");

            if (i->thread_info.muted != i->muted) {
                if(i->muted) {
                    if (pa_cvolume_is_muted(&i->thread_info.soft_volume))
                        i->thread_info.muted = i->muted;
                    else
                        pa_sink_input_set_smooth_volume(i, &i->thread_info.soft_volume, &volume);
                } else {
                    i->thread_info.muted = i->muted;
                    pa_sink_input_set_smooth_volume(i, &volume, &i->thread_info.soft_volume);
                }
                pa_sink_input_request_rewind(i, 0, TRUE, FALSE, FALSE);
            }
        }
