...
#ifdef __TIZEN_BT__
    pa_log_info("Wait for sync with bluez's dbus signal");
    usleep(100000);
#endif
    pa_log_info("Audio stream torn down");
}
...
static int transport_acquire(struct userdata *u, bool optional) {
    ...
    pa_log_info("Acquiring transport %s", u->transport->path);
...
static void setup_stream(struct userdata *u) {
    ...
    pa_log_info("Stream properly set up, we're ready to roll!");
...
finish:
    pa_log_info("IO thread shutting down");
}
...
static void handle_transport_state_change(struct userdata *u, struct pa_bluetooth_transport *t) {
    ...
    if (acquire && transport_acquire(u, true) >= 0) {
        ...
        if (u->sink) {
            pa_log_info("Resuming sink %s because its transport state changed to playing", u->sink->name);
...
static int device_process_msg(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case BLUETOOTH_MESSAGE_IO_THREAD_FAILED:
            ...
            pa_log_info("Switching the profile to off due to IO thread failure.");
